...
class AuthManager extends StormAuthManager
{
    ...
    /**
     * АУТЕНТИФИКАЦИЯ С ПРОВЕРКОЙ 2FA (TOTP)
     */
    public function authenticate($credentials, $remember = false)
    {
        ...
        // Извлекаем код 2FA, если он есть
        $twoFactorCode = array_pull($credentials, 'two_factor_code');

        try {
            // Стандартная проверка логина и пароля
            $user = parent::authenticate($originalCredentials, $remember);
        } catch (\Exception $e) {
            return false;
        }
        ...
        if ($user && $user->two_factor_enabled) {
            if (!$twoFactorCode) {
                Flash::error('Введите код двухфакторной аутентификации');
                return false;
            }

            $google2fa = new Google2FA();
            $valid = $google2fa->verifyKey($user->google2fa_secret, $twoFactorCode);

            if (!$valid) {
                \Winter\Storm\Support\Facades\Log::warning('Неверный 2FA код для пользователя: ' . $user->login);
                Flash::error('Неверный код 2FA. Попробуйте снова.');
                return false;
            }
        }
