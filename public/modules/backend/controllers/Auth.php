...
class Auth extends Controller
{
    ...
    public function signin_onSubmit()
    {
        ...
        // === 2FA: Проверяем, нужен ли код ===
        $user = \Backend\Models\User::where('login', post('login'))->first();
        $twoFactorCode = post('two_factor_code');

        if ($user && $user->two_factor_enabled && !$twoFactorCode) {
        ...
        try {
            $user = BackendAuth::authenticate($credentials, $remember);
        } catch (\Exception $e) {
