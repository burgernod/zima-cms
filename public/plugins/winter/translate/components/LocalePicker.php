...
class LocalePicker extends ComponentBase
{
    ...
    public function onSwitchLocaleProject()
    {
        // 1) Берём нужные данные из POST
        $locale = post('locale');
        $newSlug = post('slug');
        ...
        // 2) Запоминаем старую локаль
        $this->oldLocale = $this->translator->getLocale();

        // 3) Меняем локаль на новую
        $this->translator->setLocale($locale);

        // 4) Берём текущую страницу
        $page = $this->getPage();

        // Переписываем URL страницы под нужную локаль
        // (тут подтягиваются твои /project/:slug, /ru/project/:slug, /zh-CN/project/:slug и т.п.)
        $page->rewriteTranslatablePageUrl($locale);

        // 5) Берём текущие параметры роутера
        $router = new RainRouter;
        $params = $this->getRouter()->getParameters();

        // Насильно подменяем slug на тот, что пришел из POST
        $params['slug'] = $newSlug;

        // Даём шанс внешним слушателям ещё что-то поправить, как в makeLocaleUrlFromPage
        ...
        // 6) Собираем конечный URL по уже переписанному шаблону $page->url
        $pageUrl = $router->urlFromPattern($page->url, $params);

        // 7) Добавляем query string, если есть
        $pageUrl = $this->withPreservedQueryString($pageUrl, $locale);

        // 8) ВАЖНО: getPathInLocale НЕ вызываем, потому что локализованный URL уже зашит в $page->url
        return Redirect::to($pageUrl);
    }

    public function onSwitchLocaleLocation()
    {
        $locale = post('locale');
        $newSlug = post('slug');
    ...
    public function onSwitchLocaleService()
    {
        $locale = post('locale');
        $newSlug = post('slug');
    ...
    public function onSwitchLocaleCommunity()
    {
        $locale = post('locale');
        $newSlug = post('slug');
